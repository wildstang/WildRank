...
function build_cards(robots)
{
    ...
    for (let year of years)
    {
        ...
        {
            image = `<img src="${robots[year].photos[0]}" width="500px">`
        }
        let code = `<a href="${robots[year].code}">Source Code</a>`
        let awards = robots[year].awards.map(a => `${a.event}: ${a.name}`).join('<br>')
        let card = new Card(year, `<h1>${year}</h1>${image}<br>${code}<br>${awards}`)
        if (left)
        {
