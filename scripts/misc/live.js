...
function init_page()
{
    document.getElementById('header_info').innerText = 'Live'

    past_matches_card = new WRCard('')
    next_matches_card = new WRCard('')
    rankings_card = new WRCard('')

    body.append(new WRPage('', [new WRColumn('Last 5 Matches', [past_matches_card]), new WRColumn('Next 5 Matches', [next_matches_card]), new WRColumn('Top 15', [rankings_card])]))
    ...
    key_query = `?${TBA_AUTH_KEY}=${TBA_KEY}`

    // fetch simple event teams
    fetch(`https://www.thebluealliance.com/api/v3/event/${dal.event_id}/teams/simple${build_query({[TBA_AUTH_KEY]: TBA_KEY})}`)
        .then(response => {
        ...
        .then(data => {
            ...
            if (data.length > 0)
            {
                ...
                for (let match of upcoming.slice(0, Math.min(5, upcoming.length)))
                {
                    ...
                    if (teams.includes(cfg.settings.team_number))
                    {
        ...
        .then(data => {
            ...
            if (data && data.hasOwnProperty('rankings') && data.rankings.length >= 15)
            {
                ...
                for (let team of rankings.slice(0, 15))
                {
                    ...
                    if (team_key_to_number(team.team_key) === cfg.settings.team_number)
                    {
